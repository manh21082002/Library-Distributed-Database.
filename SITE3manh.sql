...
go
--Phan manh doc
select ID_docgia, tendocgia,email,tinhtrangdocgia  into DOCGIA_SITE3 from site1.thuvien.dbo.docgia

select ID_nhanvien, tennhanvien, email, luong into NHANVIEN_SITE3 from site1.thuvien.dbo.nhanvien
go

--Phan manh ngang
select * into TACGIA_SITE3 from site1.thuvien.dbo.tacgia where mota=N'Nhà văn' and gioitinh=N'Nam'

select s.* into SACH_SITE3 from site1.thuvien.dbo.sach as s join TACGIA_SITE3 on s.ID_tacgia=TACGIA_SITE3.ID_tacgia
go
-- test các mảnh
select * from TACGIA_SITE3
select * from SACH_SITE3
select * from NHANVIEN_SITE3
select * from DOCGIA_SITE3
...
begin
	...
	begin 
		...
		begin
			...
			begin
				if exists (select ID_sach from  SACH_SITE3 where ID_sach=@idsach)
				begin
					select @soluong=soluong  from  SACH_SITE3 where ID_sach=@idsach
					if (@soluong=0)
					begin
						raiserror (N'sách này hiện không còn trong kho để cho mượn',16,1)
						rollback
...
BEGIN
    ...
    BEGIN
        INSERT INTO TACGIA_SITE3 (ID_tacgia, tentacgia, ngaysinh, ngaymat, mota, gioitinh)
        VALUES (@ID_tacgia, @tentacgia, @ngaysinh, @ngaymat, @mota, @gioitinh)
...
begin
	...
	if exists (select 1 from TACGIA_SITE3 t where t.ID_tacgia=@ID_tacgia)
	begin
		insert into SACH_SITE3(ID_sach,ID_tacgia,tensach,theloai,soluong)
...
BEGIN
	...
	if exists ( select 1 from SACH_SITE3 where ID_sach = @ID_sach)
	begin
		DELETE FROM SACH_SITE3
		WHERE ID_sach = @ID_sach
...
BEGIN
    ...
    -- TÌM đến đúng site để xoá dữ liệu 
	if exists ( select 1 from TACGIA_SITE3 where ID_tacgia = @ID_tacgia)
	begin
		DELETE FROM TACGIA_SITE3
...
begin
	...
	begin
		...
		insert into DOCGIA_SITE3(ID_docgia,tendocgia,email,tinhtrangdocgia)
		values (@ID_docgia,@tendocgia,@email,@tinhtrangdocgia)
	...
	begin
		...
		begin
			...
			begin
				...
				delete from docgia_site3 where ID_docgia=@id_docgia
...
begin
	...
	begin
		...
		insert into NHANVIEN_SITE3 (ID_nhanvien,tennhanvien,email,luong)
		values (@ID_nhanvien ,@tennhanvien ,@email,@luong)
	...
	begin
		...
		begin
			...
			begin
				...
				delete from NHANVIEN_SITE3 where ID_nhanvien=@id_nhanvien
