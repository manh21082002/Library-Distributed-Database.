...
go
--Phan mảnh dọc
select ID_docgia, tendocgia,email,tinhtrangdocgia  into DOCGIA_SITE1 from docgia

select ID_nhanvien, tennhanvien, email, luong into NHANVIEN_SITE1 from nhanvien

go
--Phân mảnh ngang
select * into TACGIA_SITE1 from tacgia where mota=N'Nhà thơ' and gioitinh=N'Nam'

select sach.* into SACH_SITE1  from sach join TACGIA_SITE1 on sach.ID_tacgia=TACGIA_SITE1.ID_tacgia

go
-- Test các mảnh 
select * from SACH_SITE1
select * from NHANVIEN_SITE1
select * from DOCGIA_SITE1
select * from TACGIA_SITE1
...
alter trigger check_MUONSACH on muonsach
instead of insert
as
begin
	begin tran
	...
	begin 
		--------check docgia
		if not exists (select * from DOCGIA_SITE1 where ID_docgia=@iddocgia)
		begin 
		...
		begin
			if 0=(select tinhtrangdocgia from DOCGIA_SITE1 where ID_docgia=@iddocgia)
			begin
		...
		------check nhan vien
		if not exists (select * from NHANVIEN_SITE1 where ID_nhanvien=@idnhanvien)
		begin 
		...
		if exists (select ID_sach  from  SACH_SITE1 where ID_sach=@idsach)
		begin
			select @soluong=soluong  from  SACH_SITE1 where ID_sach=@idsach
			if (@soluong=0) 
			begin
		...
		begin
			if exists (select ID_sach  from  SITE2.thuvien.dbo.SACH_SITE2 where ID_sach=@idsach)
			begin
				select @soluong=soluong  from  SITE2.thuvien.dbo.SACH_SITE2 where ID_sach=@idsach
				if (@soluong=0)
				begin
					raiserror (N'sách này hiện không còn trong kho để cho mượn',16,1)
					rollback
				...
				begin
					update SITE2.thuvien.dbo.SACH_SITE2 
					set soluong=soluong - 1 where ID_sach=@idsach
				end
			end
			-----------------------------------
...
insert into muonsach (ID_muonsach,ID_sach,ID_docgia,ID_nhanvien,ngaymuon,ngayhethan,tinhtrangmuonsach)
values ('MS23','S001','DG11','NV07','2023-06-18','2023-06-30',1)
select * from DOCGIA_SITE1
...
BEGIN
    ...
    IF EXISTS (
        SELECT 1
        FROM (select * from TACGIA_SITE1
		union ALL select * from SITE2.thuvien.dbo.TACGIA_SITE2 
    ...
    BEGIN
        INSERT INTO TACGIA_SITE1(ID_tacgia, tentacgia, ngaysinh, ngaymat, mota, gioitinh)
    ...
    BEGIN
        INSERT INTO SITE2.thuvien.dbo.TACGIA_SITE2 (ID_tacgia, tentacgia, ngaysinh, ngaymat, mota, gioitinh)
        VALUES (@ID_tacgia, @tentacgia, @ngaysinh, @ngaymat, @mota, @gioitinh)
...
begin
	if exists (SELECT 1
        FROM (select * from SACH_SITE1
		union ALL select * from SITE2.thuvien.dbo.SACH_SITE2 
	...
	if exists (select 1 from TACGIA_SITE1 t where t.ID_tacgia=@ID_tacgia)
	begin
		insert into SACH_SITE1(ID_sach,ID_tacgia,tensach,theloai,soluong)
		values (@ID_sach,@ID_tacgia,@tensach,@theloai,@solong)
		PRINT N'Dữ liệu đã được chèn thành công vào site 1'
	end

	else if exists (select 1 from SITE2.thuvien.dbo.TACGIA_SITE2 t where t.ID_tacgia=@ID_tacgia)
	begin
		insert into SITE2.thuvien.dbo.SACH_SITE2(ID_sach,ID_tacgia,tensach,theloai,soluong)
		values (@ID_sach,@ID_tacgia,@tensach,@theloai,@solong)
		PRINT N'Dữ liệu đã được chèn thành công vào site 2'
	end
...
BEGIN
    ...
    BEGIN
        RAISERROR(N'Sách đang cho mượn không xoá được!', 16, 1)
        RETURN
    END

    -- tìm đến đúng site để xoá dữ liệu 
	if exists ( select 1 from SACH_SITE1 where ID_sach = @ID_sach)
	begin
		DELETE FROM SACH_SITE1
		WHERE ID_sach = @ID_sach
		PRINT N'Dữ liệu đã được xóa thành công từ bảng SACH_site1'
	end
	else if exists ( select 1 from SITE2.thuvien.dbo.SACH_SITE2 where ID_sach = @ID_sach)
	begin
		DELETE FROM SITE2.thuvien.dbo.SACH_SITE2 
		WHERE ID_sach = @ID_sach
...
BEGIN
    ...
    -- TÌM đến đúng site để xoá dữ liệu 
	if exists ( select 1 from TACGIA_SITE1 where ID_tacgia = @ID_tacgia)
	begin
		DELETE FROM TACGIA_SITE1
		WHERE ID_tacgia = @ID_tacgia
		PRINT N'Dữ liệu đã được xóa thành công từ bảng TACGIA_SITE1'
	end
	else if exists ( select 1 from SITE2.thuvien.dbo.TACGIA_SITE2 where ID_tacgia = @ID_tacgia)
	begin
		DELETE FROM SITE2.thuvien.dbo.TACGIA_SITE2 
		WHERE ID_tacgia = @ID_tacgia
...
as 
begin
	if exists (select * from DOCGIA_SITE1 where ID_docgia=@ID_docgia)
	begin
	...
	begin
		insert into DOCGIA_SITE1 (ID_docgia,tendocgia,email,tinhtrangdocgia)
		...
		values (@ID_docgia,@tendocgia,@email,@tinhtrangdocgia)
		insert into  site2.thuvien.dbo.DOCGIA_SITE2(ID_docgia,diachi,sdt)
...
as
begin
	if not exists (select * from DOCGIA_SITE1 where ID_docgia=@id_docgia)
	begin
	...
	begin
		if exists (select * from muonsach where ID_docgia=@id_docgia and tinhtrangmuonsach=1) 
		begin
			...
			begin
				delete from DOCGIA_SITE1 where ID_docgia=@id_docgia
				delete from site2.thuvien.dbo.docgia_site2 where ID_docgia=@id_docgia
...
as 
begin
	if exists (select * from NHANVIEN_SITE1 where ID_nhanvien=@ID_nhanvien)
	begin
	...
	begin
		insert into NHANVIEN_SITE1 (ID_nhanvien,tennhanvien,email,luong)
		...
		insert into site2.thuvien.dbo.NHANVIEN_SITE2(ID_nhanvien,chucvu,diachi)
...
as
begin
	if not exists (select * from NHANVIEN_SITE1 where ID_nhanvien=@id_nhanvien)
	begin
	...
	begin
		...
		begin
			...
			begin
				delete from NHANVIEN_SITE1 where ID_nhanvien=@id_nhanvien
				delete from site2.thuvien.dbo.NHANVIEN_SITE2 where ID_nhanvien=@id_nhanvien
