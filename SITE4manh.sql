...
go
--Phan manh doc
select ID_docgia, diachi,sdt  into DOCGIA_SITE4 from site1.thuvien.dbo.docgia

select ID_nhanvien, chucvu, diachi into NHANVIEN_SITE4 from site1.thuvien.dbo.nhanvien
go
--Phan manh ngang
select * into TACGIA_SITE4 from site1.thuvien.dbo.tacgia where mota=N'Nhà văn' and gioitinh=N'Nữ'

select s.* into SACH_SITE4 from site1.thuvien.dbo.sach as s join TACGIA_SITE4 on s.ID_tacgia=TACGIA_SITE4.ID_tacgia

go
--test các mảnh
select * from TACGIA_SITE4
select * from SACH_SITE4
select * from NHANVIEN_SITE4
select * from DOCGIA_SITE4
go
...
begin
	...
	begin 
		if not exists (select * from DOCGIA_SITE3 where ID_docgia=@iddocgia)
		begin 
		...
		------check nhan vien
		if not exists (select * from NHANVIEN_SITE3 where ID_nhanvien=@idnhanvien)
		begin 
		...
		begin
			...
			begin
				...
				begin
					if exists (select ID_sach from SACH_SITE4 where ID_sach=@idsach)
					begin
						select @soluong=soluong  from  SACH_SITE4 where ID_sach=@idsach
						if (@soluong=0)
						begin
							raiserror (N'sách này hiện không còn trong kho để cho mượn',16,1)
							rollback
...
insert into muonsach (ID_muonsach,ID_sach,ID_docgia,ID_nhanvien,ngaymuon,ngayhethan,tinhtrangmuonsach)
values ('MS21','S001','DG13','NV07','2023-06-18','2023-06-30',0)
...
BEGIN
    ...
    BEGIN
        INSERT INTO TACGIA_SITE4 (ID_tacgia, tentacgia, ngaysinh, ngaymat, mota, gioitinh)
        VALUES (@ID_tacgia, @tentacgia, @ngaysinh, @ngaymat, @mota, @gioitinh)
...
begin
	...
	IF not EXISTS (
		...
		union ALL select * from SITE3.thuvien.dbo.TACGIA_SITE3 
		union ALL select * from SITE1.thuvien.dbo.TACGIA_SITE1 ) as s
        WHERE s.ID_tacgia = @ID_tacgia
    )
    BEGIN
	...
	if exists (select 1 from TACGIA_SITE4 t where t.ID_tacgia=@ID_tacgia)
	begin
		insert into SACH_SITE4(ID_sach,ID_tacgia,tensach,theloai,soluong)
...
BEGIN
	IF not EXISTS (
			...
			union ALL select * from SITE2.thuvien.dbo.SACH_SITE2 
			union ALL select * from SITE3.thuvien.dbo.SACH_SITE3 ) as s
			WHERE s.ID_sach = @ID_sach
		)
		begin
	...
	if exists ( select 1 from SACH_SITE4 where ID_sach = @ID_sach)
	begin
		DELETE FROM SACH_SITE4
		WHERE ID_sach = @ID_sach
...
BEGIN
		IF not EXISTS (
			...
			FROM (select * from TACGIA_SITE4
				union ALL select * from SITE1.thuvien.dbo.TACGIA_SITE1
			union ALL select * from SITE2.thuvien.dbo.TACGIA_SITE2 
			union ALL select * from SITE3.thuvien.dbo.TACGIA_SITE3 ) as s
			WHERE s.ID_tacgia = @ID_tacgia
		)
		begin
    ...
    -- TÌM đến đúng site để xoá dữ liệu 
	if exists ( select 1 from TACGIA_SITE4 where ID_tacgia = @ID_tacgia)
	begin
		DELETE FROM TACGIA_SITE4
...
begin
	...
	begin
		...
		insert into DOCGIA_SITE4(ID_docgia,diachi,sdt)
		values (@ID_docgia,@Diachi,@sdt)
...
create procedure delete_docgia @id_docgia varchar(4)--not done
as
begin
	...
	begin
		...
		begin
			...
			begin
				...
				delete from docgia_site4 where ID_docgia=@id_docgia
...
begin
	...
	begin
		...
		insert into NHANVIEN_SITE4(ID_nhanvien,chucvu,diachi)
		values (@Id_nhanvien,@chucvu,@Diachi)
	...
	begin
		...
		begin
			...
			begin
				...
				delete from NHANVIEN_SITE4 where ID_nhanvien=@id_nhanvien
